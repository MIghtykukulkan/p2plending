...
function User_login(params) {
    return new Promise(function(resolve, reject){
          var ui_login;
        try{
            logHelper.logEntryAndInput(logger, 'get login details try walaaa', params);
            ...
            var passpin = params.ui_login.passpin;
            if(!validate.isValidString(passpin)){
                logHelper.logError(logger, 'login details', 'Invalid passpin');
                return reject({statusCode: constants.INVALID_INPUT, body: 'Could not fetch user details. Invalid passpin' })
            }
